...
import { CollectionType, ImageType, ProductType, ProductCollectionType } from "@/app/lib/types";
import { message } from "antd";
import { ProductStatusType } from "@/app/lib/types";
...
interface EditProductPropos {
  params: {
    id: string;
  };
}

const EditProduct: React.FC<EditProductPropos> = ({ params }) => {
  ...
  const fetchProductDetails = async () => {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        form.setFieldsValue({
          ...res.data.data,
          is_recommended: res.data.data.is_recommended === 1 ? true : false,
          collections: res.data.data.collections.map((item: ProductCollectionType) => item.collectionId),
        });
  ...
  const onFinish = async (values: FormValueType) => {
    ...
    setLoading(true);
    try {
      const res = await axios.put(`/api/admin/products/${params.id}`, newProduct);
      if (res.status === 200) {
